...
mwan3_set_policy()
{
	...
	if [ $is_lowest -eq 1 ]; then
		...
		$IPT -A "mwan3_policy_$policy" \
		     ...
		     -j MARK --set-xmark $(mwan3_id2mask id MMX_MASK)/$MMX_MASK
		[ -n "$lr_mark" ] && $IPT -A "mwan3_policy_$policy" \
		     -m mark --mark 0x0/$MMX_MASK \
		     -m comment --comment "$lr_comment" \
		     -j MARK --set-xmark $lr_mark/$MMX_MASK
		...
		# routing to table 220 (empty) and leaking to main table.
		if [ -f /tmp/mwan3_policy_has_online.$$ ]; then
...
mwan3_create_policies_iptables()
{
	...
	rm -f /tmp/mwan3_policy_has_online.$$
